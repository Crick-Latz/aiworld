...
func choose_and_offer(
	...
	var decision: Dictionary = request_policy.choose_target(request, candidate_beliefs, now_tick, seed)
	if not bool(decision.get("ok", false)):
		return decision
	var offer: Dictionary = request_policy.build_offer(request, decision, exchange_offer)
...
func answer_request(
	...
	if request.is_empty():
		return {"ok": false, "reason": "REQUEST_NOT_FOUND"}
	var response: Dictionary = response_policy.evaluate(request, recipient_context, roll)
	var recorded: bool = tracker.record_response(
		request_id,
		responder_id,
...
func accept_counter(request_id: String, tick: int) -> Dictionary:
	var accepted: bool = tracker.accept_counter(request_id, tick)
	return {
		"ok": accepted,
...
func transfer_and_resolve(
	...
	var transfer: Dictionary = transfer_service.execute(request, giver_inventory, receiver_inventory, tick)
	if not bool(transfer.get("ok", false)):
		return transfer
	var event: Dictionary = transfer.get("event", {})
	var resolved: bool = tracker.record_transfer_evidence(request_id, event, tick)
	return {
		"ok": resolved,
