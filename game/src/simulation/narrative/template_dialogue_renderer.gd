...
static func render(speech_act: Dictionary, speaker_name: String, language: String = "zh") -> Dictionary:
	...
	match act:
		"ASK_REASON":
			text = "你为什么这么做？" if anger < 0.3 else "你到底为什么？"
		"ANSWER_REASON":
		...
		"REFUSE_REQUEST":
			...
			if reason != "" and reason.find("不够") != -1:
				text = "不行，我自己也不够吃。" if politeness < 0.6 else "我也想帮你，可现在真的拿不出来。"
			else:
				text = "我不能答应。"
		"THANK":
			text = "谢谢。"
	...
	return {"ok": true, "text": "%s%s" % [speaker_name, text] if language == "zh" else text,
